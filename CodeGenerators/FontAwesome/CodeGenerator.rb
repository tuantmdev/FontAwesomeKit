class CodeGenerator
  ...
  def generate_header
    ...
    @names.each do |name|
      header_template = <<EOT
+ (instancetype)#{name}IconWithSize:(CGFloat)size style:(FAKFontAwesomeStyle)style;
EOT
...
+ (instancetype)#{name}IconWithSize:(CGFloat)size style:(FAKFontAwesomeStyle)style { return [self iconWithCode:@"#{@codes[index]}" size:size style:style]; }
EOT
